...
class CountryController extends Controller
{
    ...
    public function show(Country $country): View
    {
        ...
        foreach(Country::find($country->id)->langues()->orderBy('pourcentage','desc')->get() as $langue)
            echo $langue->nom, ': ', $langue->pivot->pourcentage, "\n";
        //print_r($langues);

        $cities = Country::find($country->id)->cities->toQuery()->orderBy('population', 'desc')->paginate(5);
        //print_r($cities);
        return view('countries.show',compact('country','cities','langues'));
    }
